...
- [#327](https://github.com/moonwell-fi/moonwell-sdk/pull/327) [`b57b7eb0d882d7d1357a0a8ae39d9d82714f5fd4`](https://github.com/moonwell-fi/moonwell-sdk/commit/b57b7eb0d882d7d1357a0a8ae39d9d82714f5fd4) Thanks [@bprofiro](https://github.com/bprofiro)! - `getUserBalances` no longer reports a failed balance read as a zero balance (MOO-832). Previously an RPC failure on a single `balanceOf` / `getBalance` call resolved to `{ amount: 0n }`, so consumers received a "successful" response in which a funded wallet read as empty — the Moonwell frontend gates repay-all on that figure, and one transient RPC hiccup during a refetch left the confirm button disabled for 59 users (Sentry MOONWELL-FRONTEND-195).
  ...
  The `onError` context shape is now exported as `OnErrorContext` from the environments module and shared by `createEnvironmentConfig`, `Environment`, and the client-level `createMoonwellClient` callback, so `context.token` is typed at every level a consumer can attach the callback.

## 0.22.0

### Minor Changes

- [#324](https://github.com/moonwell-fi/moonwell-sdk/pull/324) [`2cf9ba8d0474330ff88826e89a5847ad4f84b0d0`](https://github.com/moonwell-fi/moonwell-sdk/commit/2cf9ba8d0474330ff88826e89a5847ad4f84b0d0) Thanks [@bprofiro](https://github.com/bprofiro)! - **Breaking:** remove the sunset Moonbeam and Moonriver chains (MOO-551). Both chains are halted and their RPC endpoints are being decommissioned, so nothing on-chain can be read for them any more.

  Governance history is preserved. `getProposals`, `getProposal` and `getUserVoteReceipt` now source every governance chain — Ethereum (1), Moonbeam (1284) and Moonriver (1285) — from the governance indexer via whichever environment is registered, preferring the Ethereum multigov hub. Previously these actions required a Moonbeam or Moonriver environment and returned `[]` / `undefined` without one, so removing the chains would otherwise have emptied the entire proposal surface.

  On `getProposal` and `getUserVoteReceipt`, `chainId` now names only the chain the proposal lives on — it no longer selects the environment to read through. For `getUserVoteReceipt` this is what keeps archive receipts readable: a `chainId: 1284` lookup previously resolved no environment post-sunset and returned `[]`, a third shape alongside the didn't-vote stub and `GovernorNotFoundError`, so every vote on a historical MIP would have read as "didn't vote". Its didn't-vote stub reports the requested `chainId` when one is given, and the indexer source's chain otherwise.
...
- [#324](https://github.com/moonwell-fi/moonwell-sdk/pull/324) [`743d03c228e6daa7c34e97fe777c097e27f5e273`](https://github.com/moonwell-fi/moonwell-sdk/commit/743d03c228e6daa7c34e97fe777c097e27f5e273) Thanks [@bprofiro](https://github.com/bprofiro)! - Skip malformed Lunar indexer market records instead of rejecting the whole chain's market list. A record with missing numeric fields produced `NaN`, which `BigInt()` rejects with a RangeError that previously took down every market on the chain (observed chain-wide on the sunset Moonbeam deployment). Valid records are now kept; when every record is malformed, `getMarketsData` reports one aggregate error and falls back to on-chain reads.
  ...
  A malformed _incentive_ costs only its own reward entry — the market is still returned with its remaining rewards — rather than discarding the whole record, so a rewards-only indexer incident degrades APRs instead of emptying the chain.

...
Object.entries(testClient.environments).forEach(([networkKey, environment]) => {
  ...
  describe(`Testing user position on ${chain.name}`, () => {
    test.each(Object.keys(markets))(
      "Get market user position: %s",
      async (marketKey) => {
        const userPositionData = await testClient.getUserPosition<typeof chain>(
          {
    ...
    test.each(Object.entries(markets))(
      "Get market user position by market address: %s",
      async (_, market) => {

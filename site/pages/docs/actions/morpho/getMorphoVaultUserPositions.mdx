...
The promise rejects with a `ChainReadError` when the read fails on any requested chain, so a failed RPC never looks like an empty position list. `error.failures` lists each failed chain as `{ chainId, reason }` with the original error, `error.data` carries the positions from the chains that succeeded, and `error.source` is `"getMorphoVaultUserPositions"`. The failure is not additionally reported through the client's `onError` callback. Chains without the relevant deployment are skipped, and an account with no positions resolves normally.

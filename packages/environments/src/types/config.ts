...
export type TokensConfig<tokens> = {} extends tokens ? {} : { [tokenName in keyof tokens]: TokenConfig };
export type VaultsConfig<vaults, tokens> = {} extends vaults ? {} : { [name in keyof tokens]?: VaultConfig<tokens> };
export type MarketsConfig<markets, tokens> = {} extends markets ? {} : { [name in keyof tokens]?: MarketConfig<tokens> };
export type MorphoMarketsConfig<markets, tokens> = {} extends markets ? {} : { [name in keyof markets]: MorphoMarketConfig<tokens> };
...
export type CustomConfig<custom> = {} extends custom ? {} : { [name in keyof CustomConfigType]?: CustomConfigType[name] };

export const createTokenConfig = <const tokens>(tokens: TokensConfig<Narrow<tokens>>) => tokens as Prettify<tokens>;
...
export const createEnvironmentConfig = <tokens, markets, vaults, morphoMarkets, contracts, custom>(config: {
  name: string;
  chain: Chain;
  transport: Transport;
  indexerUrl: string;
  ...
  const createContract = <const abi extends Abi>(address: Address, abi: abi) => {
    return getContract({
      address,
      abi,
      client: publicClient,
  ...
  const getTokenContract = <const abi extends Abi>(key: keyof tokens | undefined, abi: abi) => {
    if (key) {
  ...
  const vaultsContracts = Object.keys(config.vaults || {}).reduce((prev, curr: string) => {
    return {
      ...prev,
      [curr]: getTokenContract(curr as keyof tokens, TokenAbi),
    };
